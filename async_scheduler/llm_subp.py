...
def run_p(maind_addr,queue_in: Queue, queue_out: Queue, node_id: int = 0, stage: int = 0,
    ...
    if node_id == 0:
        ...
        net = LLamaStage(dmodel=256,num_heads=8,multiple_of=256,ctx_size=seq_l,n_layers=2)
        optimizer = optim.SGD(net.parameters(),lr=4e-3,momentum=0,dampening=0,weight_decay=0,nesterov=False)
        with open(f'log{node_id}.txt', 'a') as file, redirect_stdout(file):
            ...
            while self.started:
                ...
                if isinstance(task, Start):
                    ...
                    with torch.no_grad():
                        x = x.to(self.device)
                        y = y.to(self.device)
                    self.buffer_in[task.tag[:4]] = x
                    
                    
                     
                    self.target[task.tag[:4]] = y
                    x = self.net.head(x)
                    x.retain_grad()
                    self.buffer_out[task.tag[:4]] = x
                    self.queue_out.put(Forward(pickle.dumps(x),task.tag))
                    continue
                elif isinstance(task, Loss):
                    if self.target.get(task.tag[:4]) == None:
                        return
                    x = pickle.loads(task.mb)
                    y = self.target[task.tag[:4]]
                    ...
                    self.optimizer.zero_grad()
                    if self.buffer_out.get(task.tag[:4]) == None:
                        return
                    output = pickle.loads(task.mb)
                    with torch.no_grad():
                        output = output.to(self.device)
                    inp_batch = self.buffer_out[task.tag[:4]]
                    ...
                    del self.buffer_in[task.tag[:4]]
                    del self.buffer_out[task.tag[:4]] 
                    cuda.empty_cache()
